...
df_detect %>%
  ggplot(aes(x=p, y=mean_nr, group=d)) +
  geom_point(aes(color=factor(d))) +
  geom_line(aes(color=factor(d))) +
  facet_grid(detection~., scales = "free_y") +
  scale_x_log10(breaks=c(10,30,100,300,1000,3000,6000)) +
  geom_linerange(aes(ymin=mean_nr-1.96*mean_se, ymax=mean_nr+1.96*mean_se, color=factor(d)), alpha = 0.8)
```
...
df_detect %>%
  ...
  geom_errorbar(aes(ymin=mean_nr-1.96*mean_se, ymax=mean_nr+1.96*mean_se)) +
  scale_x_log10(breaks=c(10,30,100,300,1000,3000,6000)) +
  facet_grid(detection~sig_p, scales="free_y")
