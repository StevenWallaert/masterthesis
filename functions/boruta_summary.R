boruta_summary <- function(results_of_experiment){
  ...
  performance_summary <- results_of_experiment %>%
    summarise(test_auc = mean(perf_test, na.rm=T), # calculates the mean auc (test, pop_l, pop_f)
              test_auc_se = sd(perf_test, na.rm = T)/sqrt(nsims),
              pop_auc = mean(perf_pop, na.rm=T),
              pop_auc_se = sd(perf_pop_lim, na.rm=T)/sqrt(nsims),
              ...
              auc_sd_se = sd(perf_test, na.rm=T)/sqrt(2*(nsims-1)),         # MCSE of SD
              auc_bias = mean(perf_test - perf_pop_lim, na.rm=T),
              auc_bias_se = sd(perf_test - perf_pop_lim, na.rm=T)/sqrt(nsims),
