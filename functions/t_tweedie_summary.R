# summary of results from t-tests with tweedie correction


t_tweedie_summary <- function(results_of_experiment){
  results_of_experiment <- results_of_experiment$t_tweedie
  nsims <- nrow(results_of_experiment)
  
  true_false_detections <- results_of_experiment %>%
    mutate(i=factor(i)) %>%
    rowwise() %>%
    mutate(detections = ifelse(is.na(perf_test), list(NA), list(detections))) %>%
    unnest(detections) %>%
    rowwise() %>%
